...
show_help({
    "목적": "월별 출고 데이터로 품목별 ABC 등급을 산출하고 재배치·히트맵 결과 xlsx를 생성.",
    "필요한 파일": "① 로케 마스터 xlsx (한 번 등록)  "
                   "② 월별 출고 xlsx (매달 등록, ERP 일평균 출고현황 원본 지원)  "
                   "③ (선택) ERP Item_*.xlsx 원본 — 현재로케·하대 일괄 갱신용",
    "사용 순서": "1. ① 로케 마스터 관리 (최초 1회 업로드, 이후 필요 시 [현재로케] · [하대] 일괄 갱신)\n"
                 "2. ② 월별 데이터 등록 — 기준월 입력 → 파일 업로드 → 신규 품번 자동 마스터 추가\n"
                 "3. ③ 분석 실행 — 기준월 선택 → [▶ ABC 재분류 + 히트맵 생성]\n"
                 "4. 📥 결과 xlsx 다운로드 (요약 · ABC 리스트 · 재배치 추천 · 히트맵 등 9시트)",
    "자동 감지": "· 신규 품번: 월 파일에 있고 마스터에 없으면 자동 추가 (출시월=등록월YYMM)\n"
                  "· 현재로케·하대: ERP Item_*.xlsx 원본 그대로 업로드 → 자동 필터·계산",
    "참고": "월별 데이터는 Supabase에 영구 저장 → 매월 새 월 파일 1회만 등록. 재배포/재부팅에도 유지.",
})
